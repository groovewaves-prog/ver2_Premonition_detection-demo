...
#### 対策A: @st.fragment によるフラグメント化
Streamlit は任意のウィジェット操作でページ全体を再実行する。
トリアージの「全コマンド一括実行」等のボタン操作が毎回全ページ再描画を引き起こしていた。

- `future_radar.py`: 全インタラクティブ部分を `@st.fragment` 化
  - トリアージボタン・一括実行ボタン → フラグメントのみ再描画
  - ヘッダーHTML構築を `_build_prediction_header_html()` に分離し事前構築
- `root_cause_table.py`: 障害時トリアージ部分を `@st.fragment` 化
- `requirements.txt`: `streamlit>=1.37.0` に引き上げ

#### 対策B: LogicalRCA.analyze() のキャッシュハッシュからINFOアラーム除外
- **根本原因**: `inference_engine.py` の `analyze()` が全アラーム（INFO含む）でハッシュを
  計算していたため、シミュレーション level 変更のたびに GrayScope/Granger/LLM が
  フル実行されていた（数秒の遅延）。
- cockpit.py 側は元々 INFO 除外ハッシュを使用しており、`analyze()` 側も統一。
- 加えて、`sidebar.py` から `_analysis_cache_*` のクリアを除去
  （prediction_pipeline.py のスライス代入で古い予測は自動除去されるため不要。
  クリアすると analyze() フル実行で遅延が発生）。

#### 効果
- トリアージボタン操作: 全ページ再描画 → フラグメント（数十ms）のみ
- シミュレーション level 変更: analyze() フル実行（数秒）→ キャッシュヒット（即座）
