...
### 9. 障害シナリオ切替時の描画高速化
- **問題**: 障害シナリオ設定後、cockpit.py が全root_cause候補のLLMトリアージを同期ループで生成 → N候補 × 3-8秒 = 15-40秒のブロック
- **修正**: cockpit.py の一括生成ループ（旧 lines 516-596）を廃止
- **代替**: root_cause_table.py で選択された候補のみオンデマンド生成（`_generate_incident_triage_lazy()`）
  - スピナー表示付きで1件だけLLM呼び出し（3-5秒）
  - 結果は session_state にキャッシュ → 次回以降は即座に返却
  - 候補切替時も必要な分だけ生成される
- **効果**: シナリオ切替直後の描画が即座に完了し、トリアージは選択候補表示時のみ生成
